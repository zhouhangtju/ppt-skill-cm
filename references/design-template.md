...
This file captures China Mobile's preferred visual and content template for technical research PPTs.
...
- benchmark style: closer to China Mobile's mature research decks than to generic consulting minimalism
...
- when no stronger user reference exists, the standard chapter-directory sidebar blue should also default to `#1E88D8`; do not arbitrarily deepen it into navy / slate / teal variants
...
- `Microsoft YaHei` is the default standard for Chinese PPT content in this skill, not only a loose preference
- if the implementation route uses a builder or library that can silently retain Office default theme fonts, explicitly verify and, if necessary, repair the exported `.pptx` so the final file does not fall back to `Calibri`, `Calibri Light`, `Aptos`, or similar defaults for Chinese text
...
Observed preference from China Mobile's own sample decks:
...
### Variant A: Standard blue sidebar directory
...
Required traits:
- left standard-blue sidebar panel, defaulting to `#1E88D8` unless a stronger user reference clearly overrides it
...
- do not darken the sidebar into a deeper custom blue without an explicit style reason from the user or reference deck
